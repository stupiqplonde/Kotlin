...
data class VictoryAchieved(override val playerId: String, val questId: String) : GameEvent
            ...
            is CmdChooseDialogueOption -> {
                ...
                when (cmd.optionId) {
                    ...
                    "give_herb" -> {
                        ...
                        _events.emit(VictoryAchieved(cmd.playerId, "alchemist_herb_quest"))
...
class HudState {
    ...
    val showVictoryOverlay = mutableStateOf(false)
    val confettiTick = mutableStateOf(0)
    val confetti = List(36) { idx ->
        val lane = idx % 6
        val phase = (idx * 7) % 24
        val symbol = when (idx % 5) {
            0 -> '*'
            1 -> '+'
            2 -> 'o'
            3 -> '@'
        ...
        ConfettiParticle(lane = lane, phase = phase, symbol = symbol)
    }
...
fun eventToText(e: GameEvent): String = when (e) {
    ...
    is VictoryAchieved        -> "VictoryAchieved ${e.questId}"
...
fun main() = KoolApplication {
    ...
    addScene {
        ...
        hud.activePlayerIdFlow
            ...
            .onEach {
                hud.playerSnapShot.value = it
                hud.showVictoryOverlay.value = it.questState == QuestState.GOOD_END && it.alchemistMemory.receivedHerb
                if (!hud.showVictoryOverlay.value) {
                    hud.confettiTick.value = 0
                }
            }
        ...
        hud.activePlayerIdFlow
            ...
            .onEach {
                if (it is VictoryAchieved) {
                    hud.showVictoryOverlay.value = true
                    hud.confettiTick.value = 0
                }
                hudLog(hud, "[${hud.activePlayerIdUi.value}] ${eventToText(it)}")
            }
            .launchIn(coroutineScope)

        onUpdate {
            if (hud.showVictoryOverlay.value) {
                hud.confettiTick.value += 1
            }
        }
        ...
        addPanelSurface {
            val showVictory = hud.showVictoryOverlay.use()
            if (!showVictory) {
                return@addPanelSurface
            }

            val tick = hud.confettiTick.use()
            modifier
                .align(AlignmentX.Center, AlignmentY.Center)
                .width(100.dp)
                .height(100.dp)
                .background(RoundRectBackground(Color(0f, 0f, 0f, 0.88f), 0.dp))
                .padding(20.dp)
            ...
            Column {
                ...
                Text("Квест успешно сдан алхимику") {
                    modifier.margin(bottom = sizes.gap)
                }

                for (lineIdx in 0 until 6) {
                    val lineParticles = hud.confetti.filter { it.lane == lineIdx }
                    val confettiLine = buildString {
                        var cursor = 0
                        val width = 52
                        for (particle in lineParticles) {
                            val x = (tick + particle.phase * 3 + lineIdx * 5) % width
                            val gap = if (x >= cursor) x - cursor else width - cursor + x
                            repeat(gap) { append(' ') }
                            append(particle.symbol)
                            cursor = (x + 1) % width
                        }
                    }
                    Text(confettiLine) {
