...
class HudState{
    ...
    val createdCubesUi = mutableStateOf(0)
}

class CubeSpawnerState {
    private val _createdCubes = MutableStateFlow(0)
    val createdCubes: StateFlow<Int> = _createdCubes.asStateFlow()
    private val _cubeHeights = MutableStateFlow<List<Float>>(emptyList())
    val cubeHeights: StateFlow<List<Float>> = _cubeHeights.asStateFlow()

    private val _events = MutableSharedFlow<String>(extraBufferCapacity = 64)
    val events: SharedFlow<String> = _events.asSharedFlow()

    fun nextCubeY(): Float {
        return (_cubeHeights.value.lastOrNull() ?: 0f) + 1f
    }

    fun onSpawnCube(y: Float) {
        _cubeHeights.value = _cubeHeights.value + y
        _createdCubes.value = _cubeHeights.value.size
        _events.tryEmit("Создан куб #${_createdCubes.value} в координатах [0, $y, 0]")
    }

    fun restoreCubes(heights: List<Float>) {
        _cubeHeights.value = heights
        _createdCubes.value = heights.size
        _events.tryEmit("Загружено кубов из сохранения: ${heights.size}")
    }

    fun onKeyPressedE() {
        _events.tryEmit("Нажата клавиша E")
    }
}

@Serializable
data class CubeSaveData(
    val yHeights: List<Float>
)

object CubeSaveStorage {
    private val json = Json {
        prettyPrint = true
        ignoreUnknownKeys = true
    }

    private val saveFile = File("cubeStack-save.json")

    fun loadHeights(): List<Float> {
        if (!saveFile.exists()) return emptyList()
        return runCatching {
            json.decodeFromString<CubeSaveData>(saveFile.readText()).yHeights
        }.getOrElse { emptyList() }
    }

    fun saveHeights(heights: List<Float>): Boolean {
        return runCatching {
            val payload = json.encodeToString(CubeSaveData(heights))
...
fun addStackCube(scene: Scene, y: Float) {
    scene.addColorMesh {
...
fun main() = KoolApplication {
    ...
    val cubeSpawner = CubeSpawnerState()
    val maxHp = 100

    addScene {
        val worldScene = this
        var pendingCubeSpawns = 0
        defaultOrbitCamera()
        ...
        addColorMesh {
            ...
            transform.translate(-3f, 3f, -3f)
        }

        val restoredCubeHeights = CubeSaveStorage.loadHeights()
        for (y in restoredCubeHeights) {
            addStackCube(worldScene, y)
        }
        cubeSpawner.restoreCubes(restoredCubeHeights)

        KeyboardInput.addKeyListener(
            keyCode = UniversalKeyCode('E'),
            name = "cube-spawn-on-e"
        ) { keyEvent ->
            cubeSpawner.onKeyPressedE()
            pendingCubeSpawns += 1
        }

        onUpdate {
            while (pendingCubeSpawns > 0) {
                pendingCubeSpawns -= 1
                val cubeY = cubeSpawner.nextCubeY()
                addStackCube(worldScene, cubeY)

                cubeSpawner.onSpawnCube(cubeY)
                val saved = CubeSaveStorage.saveHeights(cubeSpawner.cubeHeights.value)
                if (saved) {
                    hudLog(hud, "Сохранение кубов обновлено (cubeStack-save.json)")
                } else {
                    hudLog(hud, "Ошибка сохранения cubeStack-save.json")
                }
            }
        }
...
//         строим пол из мелких кубов
    ...
    addScene {
        ...
        cubeSpawner.createdCubes
            .onEach { count -> hud.createdCubesUi.value = count }
            .launchIn(coroutineScope)
        ...
        addPanelSurface {
            ...
            Column {
                ...
                Text("Created cubes: ${hud.createdCubesUi.use()}") {}
                modifier.margin(bottom = sizes.gap)

                Button("Take Damage") {
                    modifier.margin(end = 8.dp).onClick {
                        hudLog(hud, "Отправлена команда CmdTakeDamage для ${player.playerId}")
        ...
        addPanelSurface {
            ...
            Column {
                ...
                val logLines = hud.log.use()
                for (line in logLines.takeLast(8)) {
                    Text(line) {}
